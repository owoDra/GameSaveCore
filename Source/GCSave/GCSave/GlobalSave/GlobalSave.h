...
class GCSAVE_API UGlobalSave : public USaveGame
{
public:
	GENERATED_BODY()
public:
...
public:
	/** 
	 * Returns the game instance this is associated with, this will be valid if it is ready to save 
	 */
	UFUNCTION(BlueprintCallable, Category = "Save Game|Info")
	virtual UGameInstance* GetGameInstance() const { return OwningGameInstance.Get(); }
	...
	static FString GetDefaultSaveSlotName(TSubclassOf<UGlobalSave> GlobalSaveClass) 
	{ 
		return GlobalSaveClass ? GlobalSaveClass.GetDefaultObject()->GetSaveSlotName() : FString();
	}
...
public:
	 ...
	 */
	virtual void InitializeSaveGame(UGameInstance* GameInstance, FString InSlotName);
